target_y = (-90, -146)
target_x = (102, 157)
#given an initial velocity, returns True if probe hits target, False if probe doesn't hit target 
...
min_y = -475
max_y = target_y[1]
for i in range(-475, 475):
    for j in range(-475, 475):
        t = hitsTarget((0, 0), (i, j), target_x, target_y)
        if t: 
            max_y = min_y
